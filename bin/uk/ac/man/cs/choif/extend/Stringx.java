...
/**
 * Given a document as a list of sentences,
 * convert it to a list of tokenized sentences.
 * Creation date: (11/30/99 13:14:00)
 * @return java.lang.String[][]
 * @param D java.lang.String[]
 * @param delimiters java.lang.String
 */
public final static String[][] tokenize(String[] D, String delimiters) {
	String[][] R = new String[D.length][];
	Vector V = new Vector(100);
	StringTokenizer tk;
	for (int i=D.length; i-->0;) {
		tk = new StringTokenizer(D[i], delimiters, false);
		while (tk.hasMoreTokens()) V.addElement(tk.nextToken());
		R[i] = new String[V.size()];
		V.copyInto(R[i]);
		V.removeAllElements();
	}
	return R;
